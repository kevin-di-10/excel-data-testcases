...
def generate_data_conversion(rows=20000):
    ...
    for i in range(rows):
        ...
        if random.random() > 0.5:
            ...
            percentage_str = f"{percentage/100:.3f}"
...
def generate_text_split_data(rows=12000):
    ...
    areas = ['望京', '陆家嘴', '华侨城', '西湖', '新街口']
    
    for i in range(rows):
        province = random.choice(provinces)
        city = random.choice(cities_map[province])
...
def main():
    """主函数 - 生成所有测试数据"""
    print("=" * 60)
    print("Excel 数据处理测试用例生成器")
    print("=" * 60)
    print()
    
    datasets = [
        ('1_data_cleaning.csv', generate_data_cleaning(15000), '数据清洗'),
        ('2_data_conversion.csv', generate_data_conversion(20000), '数据转换'),
        ('3_data_merge_orders.csv', generate_data_merge_orders(25000), '订单关联'),
        ('3_data_merge_customers.csv', generate_data_merge_customers(10000), '客户关联'),
        ('4_pivot_table_data.csv', generate_pivot_table_data(30000), '数据透视'),
        ('5_text_split.csv', generate_text_split_data(12000), '文本分割'),
        ('6_anomaly_detection.csv', generate_anomaly_detection_data(18000), '异常检测'),
    ...
    for filename, df, description in datasets:
        ...
        total_rows += rows
        total_size += size
        print(f"✓ {filename:30s} | {rows:>6,} 行 | {size:>6.2f} MB | {description}")
    
    print()
    print("=" * 60)
    print(f"总计: {total_rows:,} 行数据 | {total_size:.2f} MB")
    print("=" * 60)
    print("\n✓ 所有测试数据生成完成！")
    print("\n提示: 使用以下命令查看数据:")
    print("  pandas: pd.read_csv('filename.csv')")
    print("  R:      read.csv('filename.csv')")
    print("  Excel:  直接打开CSV文件\n")
